...
// Curated cipher suites to probe, with a rough security strength.
const CIPHER_SUITES = [
  { name: 'TLS_AES_256_GCM_SHA384', v: 'TLSv1.3', strength: 'secure' },
  { name: 'TLS_AES_128_GCM_SHA256', v: 'TLSv1.3', strength: 'secure' },
  { name: 'TLS_CHACHA20_POLY1305_SHA256', v: 'TLSv1.3', strength: 'secure' },
  { name: 'ECDHE-RSA-AES256-GCM-SHA384', v: 'TLSv1.2', strength: 'secure' },
  { name: 'ECDHE-RSA-AES128-GCM-SHA256', v: 'TLSv1.2', strength: 'secure' },
  { name: 'ECDHE-ECDSA-AES256-GCM-SHA384', v: 'TLSv1.2', strength: 'secure' },
  { name: 'ECDHE-ECDSA-AES128-GCM-SHA256', v: 'TLSv1.2', strength: 'secure' },
  { name: 'ECDHE-RSA-CHACHA20-POLY1305', v: 'TLSv1.2', strength: 'secure' },
  { name: 'ECDHE-ECDSA-CHACHA20-POLY1305', v: 'TLSv1.2', strength: 'secure' },
  { name: 'DHE-RSA-AES256-GCM-SHA384', v: 'TLSv1.2', strength: 'secure' },
  { name: 'DHE-RSA-AES128-GCM-SHA256', v: 'TLSv1.2', strength: 'secure' },
  { name: 'ECDHE-RSA-AES256-SHA384', v: 'TLSv1.2', strength: 'moderate' },
  { name: 'ECDHE-RSA-AES128-SHA256', v: 'TLSv1.2', strength: 'moderate' },
  { name: 'ECDHE-ECDSA-AES256-SHA384', v: 'TLSv1.2', strength: 'moderate' },
  { name: 'ECDHE-ECDSA-AES128-SHA256', v: 'TLSv1.2', strength: 'moderate' },
  { name: 'DHE-RSA-AES256-SHA256', v: 'TLSv1.2', strength: 'moderate' },
  { name: 'DHE-RSA-AES128-SHA256', v: 'TLSv1.2', strength: 'moderate' },
  { name: 'AES256-GCM-SHA384', v: 'TLSv1.2', strength: 'weak' },
  { name: 'AES128-GCM-SHA256', v: 'TLSv1.2', strength: 'weak' },
  { name: 'AES256-SHA256', v: 'TLSv1.2', strength: 'weak' },
  { name: 'AES128-SHA256', v: 'TLSv1.2', strength: 'weak' },
  { name: 'ECDHE-RSA-AES256-SHA', v: 'TLSv1.2', strength: 'insecure' },
  { name: 'ECDHE-RSA-AES128-SHA', v: 'TLSv1.2', strength: 'insecure' },
  { name: 'AES256-SHA', v: 'TLSv1.2', strength: 'insecure' },
  { name: 'AES128-SHA', v: 'TLSv1.2', strength: 'insecure' },
  { name: 'DES-CBC3-SHA', v: 'TLSv1.2', strength: 'insecure' },
  { name: 'RC4-SHA', v: 'TLSv1.2', strength: 'insecure' },
];

function isPfs(name, version) {
  if (version === 'TLSv1.3') return true; // all TLS 1.3 suites use (EC)DHE
  return /ECDHE|DHE/i.test(name);
}

function testCipher(host, port, cipher) {
  return new Promise((resolve) => {
    let done = false;
    const finish = (r) => { if (!done) { done = true; resolve(r); } };
    let socket;
    try {
      const opts = { host, port, servername: host, ciphers: cipher.name, rejectUnauthorized: false, timeout: 2000 };
      if (cipher.v === 'TLSv1.2') { opts.minVersion = 'TLSv1.2'; opts.maxVersion = 'TLSv1.2'; }
      else if (cipher.v === 'TLSv1.3') { opts.minVersion = 'TLSv1.3'; opts.maxVersion = 'TLSv1.3'; }
      socket = tls.connect(opts);
    } catch {
      return finish(false); // cipher unavailable in this OpenSSL build (e.g. RC4/DES)
    }
    socket.once('secureConnect', () => { finish(true); socket.destroy(); });
    socket.once('error', () => { finish(false); socket.destroy(); });
    socket.once('timeout', () => { finish(false); socket.destroy(); });
  });
}

/** Enumerate which cipher suites the server negotiates. */
async function checkCipherSuites(host, port) {
  const results = await Promise.all(CIPHER_SUITES.map(async (cipher) => {
    const supported = await testCipher(host, port, cipher);
    return { name: cipher.name, version: cipher.v, strength: cipher.strength, pfs: isPfs(cipher.name, cipher.v), supported };
  }));
  return results.filter((r) => r.supported);
}

/** Check for the HTTP Strict-Transport-Security header. */
function checkHsts(host) {
  return new Promise((resolve) => {
    let done = false;
    const finish = (r) => { if (!done) { done = true; resolve(r); } };
    const req = https.request({
      host, port: 443, path: '/', method: 'HEAD',
      servername: host, rejectUnauthorized: false, timeout: 3000,
    }, (res) => {
      const hsts = res.headers['strict-transport-security'];
      res.resume();
      finish({ found: !!hsts, value: hsts || null });
    });
    req.on('error', () => finish({ found: false, value: null }));
    req.on('timeout', () => { req.destroy(); finish({ found: false, value: null }); });
    req.end();
  });
}
...
async function checkSSL(host, port, timeout = 8000) {
  ...
  if (result.connected && result.tls_negotiated && result.certificate) {
    ...
    const [ocspR, tlsR, caaR, cipherR, hstsR] = await Promise.allSettled([
      checkOcsp(leaf, issuer),
      checkTlsVersions(host, port),
      checkCaa(host),
      checkCipherSuites(host, port),
      checkHsts(host),
    ]);
    ...
    result.cipher_suites = cipherR.status === 'fulfilled' ? cipherR.value : [];
    result.hsts = hstsR.status === 'fulfilled' ? hstsR.value : { found: false };
  }
